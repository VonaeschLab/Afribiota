...
make_biplot <- function( vectorPCOA
  ...
  #' @param mult multiplication factor for length of arrow segments (DEPRECATED)
  #' @param label_text text of color categoriy
...
deseq_fold_change <- function( dataset, level = "Genus"
  ...
  ### deseq commands
  df = dataset %>% phyloseq::tax_glom( taxrank = level )
  ...
  filtered = ordered[ ordered$padj <= sig_level
     & abs(ordered$log2FoldChange) >= fold_change , ]
