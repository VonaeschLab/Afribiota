### Analysis of 16S study Afribiota June 2021, Vonaesch et al., submitted #########
...
#### import data ####
tax = read.delim("tax_table16S_decoded.csv", header = TRUE, sep = ",")
colnames(tax)[1] = "ASV"
...
ASV = read.delim("ASV_table16S_decoded.csv", header = TRUE, sep = ";")
colnames(ASV)[1] = "ASV"
...
meta   = read.delim("metadata16S_decoded.csv", header = TRUE, sep = ";")

dim(meta) #1172 samples and 65 metadata categories
...
#### clean up metadata and keep only samples with at least 5000 seq ####
...
phyloseq::sample_data(df2)$ageyears = factor(phyloseq::sample_data(df2)$ageyears
                                             , labels = c("2-3 years", "3-4 years", "4-5 years"))
phyloseq::sample_data(df2)$haz      = as.factor(phyloseq::sample_data(df2)$haz)
phyloseq::sample_data(df2)$haz      = factor(phyloseq::sample_data(df2)$haz
                                             , labels = c("non-stunted", "moderately stunted", "severely stunted"))
...
#track the evolution of  sample numbers
...
pdf("samplesequeningdepthfulldataset.pdf",
  width = 10,
  height = 8)
...
#### Alpha diversity measures ####
    ...
    # make labels on variables to better interpret results
    phyloseq::sample_data(df_red)$ageyears = factor(phyloseq::sample_data(df_red)$ageyears
                                                    , labels =c("2-3 years", "3-4 years", "4-5 years"))
    phyloseq::sample_data(df_red)$haz = factor(phyloseq::sample_data(df_red)$haz
                                               , labels = c("non-stunted", "moderately stunted", "severely stunted"))
    ...
    df_red_rar_duodenal_CAR # 77 samples

    #### Make a subset of rarefied, untrimmed samples which do have all three compartments covered for untrimmed dataset CAREFUL CHANGES CHECK ####
    ...
    #### now assess the alpha diversity in the reduced dataset with only subjects with samples in all three compartments for SampleType CHANGES CHECK IN ####
...
##### loop for alpha diveristy in fecal and duodenal datasets
for( dataset_name in c("feces", "duodenal") ){
     # dataset_name = "duodenal"; jj = 3
      if (dataset_name == "feces")    dataset =  df_red_rar_feces
      if (dataset_name == "duodenal") dataset =  df_red_rar_duodenal

      out = richness_loop( dataset = dataset, variables = c("pays")
  ...
  for (jj in 1:3){
      ...
      if (jj == 1){  # no country subsetting
        df = dataset
        dname = dataset_name
      }

      if (jj == 2){ # Madagascar jj = 2
        df = dataset %>% phyloseq::subset_samples( pays == "Madagascar" )
        dname = paste0( dataset_name, "_Mada")
      }

      if (jj == 3){ # RCA
        df = dataset %>% phyloseq::subset_samples( pays == "RCA" )
        dname = paste0( dataset_name, "_RCA")
      }

      #-- need to define aatstunted !
     phyloseq::sample_data(df)$aatstunted = ""

     phyloseq::sample_data(df)$aatstunted[
      which((phyloseq::sample_data(df)$alphaantitrypsinlevel != "normal")
          & (phyloseq::sample_data(df)$haz != "non-stunted"))] <- "highAATandstunted"

    phyloseq::sample_data(df)$aatstunted[
      which((phyloseq::sample_data(df)$alphaantitrypsinlevel != "normal")
          & (phyloseq::sample_data(df)$haz == "non-stunted"))] <- "highAATnonstunted"

    phyloseq::sample_data(df)$aatstunted[
      which((phyloseq::sample_data(df)$alphaantitrypsinlevel == "normal")
          & (phyloseq::sample_data(df)$haz != "non-stunted"))] <- "normalAATandstunted"

    phyloseq::sample_data(df)$aatstunted[
      which((phyloseq::sample_data(df)$alphaantitrypsinlevel == "normal")
          & (phyloseq::sample_data(df)$haz == "non-stunted"))] <- "normalAATnonstunted"

    print( table(phyloseq::sample_data(df)$aatstunted, phyloseq::sample_data(df)$pays) )

      #-- loop over variables
      outs = richness_loop( dataset = df
                          , dataset_name = dname
                          , variables = c( "anemie","alpha_calpro_connection"
                                           , "stunted" ,"haz", "ageyears",
      ...
      #-- subset of data
      outs = richness_loop( dataset = df %>%
                              phyloseq::subset_samples( stunted == "stunted" )
                          , dataset_name = paste0( dname, "_stunted")
                          , variables = c( "anemie","sibo" )
                          , save_pdf = TRUE, write_table = TRUE )
      print(outs)

      #-- there are no samples in duodenal and RCA that are non-stunted
      if ( dataset_name == "duodenal" & jj == 3) outs = NULL else
      outs = richness_loop( dataset = df %>%
                              phyloseq::subset_samples( haz == "non-stunted" )
                          , dataset_name = paste0( dname, "_non_stunted")
                          , variables = c( "anemie" )
                          , save_pdf = TRUE, write_table = TRUE )
      print(outs)
    ...
    }# end dataset_name loop


####'  Descriptive tables and plots for the overall sample composition ####
    ...
    for (name in c( "", "reduced" ) ){
      if (name == ""       ) dataset = dffiltered4
      if (name == "reduced") dataset = dffiltered4_red
    ...
    #### calculate prevalence for Phylum, Genus, Species for fecal, duodenal, gastric samples and country ####
    for ( name in c("feces"," duodenal", "gastric")){

      if (name == "feces"   ) dataset = dffilteredfeces
      if (name == "duodenal") dataset = dffilteredduodenal
      if (name == "gastric" ) dataset = dffilteredgastric


    for ( level in  c( "Phylum" ,"Genus", "Species" )) {
    res = prevalence_abundance( dataset =  dataset
                          , level = level
                          , name = name
                          , group = "pays"
                          , write_csv = TRUE )
...
pdf("Phylumcomposition of Afribiota Samples.pdf",
  width = 30,
  height = 10)
...
pdf("AbundancetablePhylumSampleType.pdf",
  width = 20,
  height = 8)
...
pdf("AbundancetablePhylumSampleTypesharedonly.pdf",
  width = 20,
  height = 8)
...
pdf("AbundancetablePhylumfecescountryoforigin.pdf",
  width = 20,
  height = 8)
...
pdf("AbundancetablePhylumduodenalcountryoforigin.pdf",
  width = 13,
  height = 8)
...
pdf("AbundancetablePhylumduodenalsibo.pdf",
  width = 6,
  height = 8)
...
pdf("AbundancetablePhylumgastriccountryoforigin.pdf",
  width = 20,
  height = 8)
...
pdf("AbundancetablePhylumgastricstuntingstatus.pdf",
  width = 11,
  height = 8)
...
pdf("AbundancetablePhylumgastricstuntingstatusBanguionly.pdf",
  width = 11,
  height = 8)
ggplot(Afribiota16Sftgastric_p_s, aes(x =haz, y = Abundance, fill = Phylum)) +
 theme_bw() +
...
pdf("AbundancetablePhylumgastricstuntingstatusTanaonly.pdf",
  width = 15,
  height = 8)
ggplot(Afribiota16Sftgastric_p_s, aes(x =haz, y = Abundance, fill = Phylum)) +
 theme_bw() +
...
pdf("AbundancetablePhylumduodenalstuntingstatus.pdf",
  width = 11,
  height = 8)
...
# Restrict on taxa with at least an abundance of 1%

pdf("AbundancetablePhylumduodenalstuntingstatusBanguionly.pdf",
  width = 11,
  height = 8)
...
pdf("AbundancetablePhylumduodenalstuntingstatusTanaonly.pdf",
  width = 11,
  height = 8)
ggplot(Afribiota16Sftduodenal_p_s_f, aes(x =haz, y = Abundance, fill = Phylum)) +
 theme_bw() +
...
pdf("AbundancetablePhylumfecesstuntingstatus.pdf",
  width = 15,
  height = 8)
...
pdf("AbundancetablePhylumfecesstuntingstatusBanguionly.pdf",
  width = 15,
  height = 8)
...
pdf("AbundancetablePhylumfecesstuntingstatusTanaonly.pdf",
  width = 15,
  height = 8)
ggplot(Afribiota16Sftfeces_p_s_f, aes(x =haz, y = Abundance, fill = Phylum)) +
 theme_bw() +
...
pdf("AbundancetableGenusduodenalsibo.pdf",
  width = 50,
  height = 8)
...
pdf("AbundancetableGenusduodenalsibotop10.pdf",
  width = 6,
  height = 8)
...
pdf("AbundancetableGenusduodenalsiboBangui.pdf",
  width = 50,
  height = 8)
ggplot(Afribiota16Sftduodenal_p_s, aes(x =sibo, y = Abundance, fill = Genus)) +
 theme_bw() +
...
pdf("AbundancetableGenusduodenalsibotop10Bangui.pdf",
  width = 12,
  height = 8)
ggplot(duodenal10, aes(x =sibo, y = Abundance, fill = Genus)) +
 theme_bw() +
...
pdf("AbundancetableGenusduodenalsiboTana.pdf",
  width = 50,
  height = 8)
ggplot(Afribiota16Sftduodenal_p_s, aes(x =sibo, y = Abundance, fill = Genus)) +
 theme_bw() +
...
pdf("AbundancetableGenusduodenalsibotop10Tana.pdf",
  width = 12,
  height = 8)
ggplot(duodenal10, aes(x =sibo, y = Abundance, fill = Genus)) +
 theme_bw() +
...
####  Ordination plots ####
#### PERMANOVA to look for factors influencing dispersion of whole sample set ####
    ...
    pdf(file = paste0(level, "_SampleType16S_explantory_arrowsrelabun.pdf"),
      width = 8, height = 5)

    gg = original_biplot( vectorPCOA = out2$vectorPCOA
    ...
    pdf(file = paste0( level, "SampleType16S_explantory_arrowsrelabun_CAR.pdf"), width = 8, height = 5 )
    ...
    #### PERMANOVA to look for factors influencing dispersion of Mada sample set ####
    ...
    pdf(file = paste0( level, "SampleType16S_explantory_arrowsrelabun_Mada2.pdf"),
      width = 8, height = 5)
    ...
    level = "Order"
    out = setup_ordination( dataset = dffilteredfeces_rar, level = level)
    out2= setup_pcoa_scores( dist = out$dist, test4 = out$test4, meta = out$meta, sig_level = 0.01 )

    pdf(file = paste0(level, "pays_RCA_vs_Madagascar6S_fecal.pdf"),
      width = 5,
      height = 5)
   ...
   gg = original_biplot( vectorPCOA = out2$vectorPCOA
    ...
    for (i in 1:1) {
     # Subsample
     dffilteredfecesMada_rar  = phyloseq::rarefy_even_depth(dffilteredfecesMada, sample.size = 5000, verbose = FALSE, replace = TRUE)
    }
    ...
    ##### Plot Ordination with significant taxa as explanatory arrows on Order, Family level- pays ####

    for (level in c("Order" ,"Family")){

    out = setup_ordination( dataset = dffilteredduodenal_rar, level = level )
    out2= setup_pcoa_scores( dist = out$dist, test4 = out$test4, meta = out$meta, sig_level = 0.01 )

    pdf(file = paste0( level, "_pays_RCA_vs_Madagascar6S_explantory_arrowsrelabun_duodenal.pdf"), width = 8, height = 5)
    ...
    gg = original_biplot( vectorPCOA = out2$vectorPCOA
         ...
         labs( subtitle = paste("Duodenal Samples: ", level) )
    print(gg)
...
##### Analyse the core microbiota in the duodenum using the microbiome package #####
    ...
    # find taxa which are at least shared by 75% of all samples and have an abundance of at least 0.01%
...
#### make the Deseq analysis on all gastric samples and pays of origin on Genus level all gastric samples ####
    ...
    res = deseq_fold_change( dataset = dffilteredgastric, level = level
                             , factor_variables = c("sexe", "Run")
    ...
    pdf("differentially_present_taxa_Genus_bycountryoforigin_corr_gender_age_gastric_LRTallsamples.pdf",
      width = 10,
      height = 8)
    ...
    # make graph plot
    pdf("differentially_present_genera by country of origin_gastric samples.pdf",
      width = 8,
      height = 6)
    ...
    res = dffilteredgastric  %>%
             ...
             filter_prune( ) %>%
             deseq_fold_change( level = level
                             , factor_variables = c("sexe", "Run", "haz", "pays")
                             , continuous_variables = "age"
                             , variable_of_interest = variable_of_interest
                             , sig_level = 0.05)
    ...
    merged   = prevalence_abundance( dataset = res$glom
                                   , level = level , name = name
                                   , group = variable_of_interest, write_csv = TRUE )
    sibocorr = deseq_names( res, merged, name = name
                            , fold_change = 0
                            , sig_level = 0.01
                            , write_csv = TRUE ) %>% max_reorder_results()
    ...
    pdf("differentially_present_taxa_Species_bysibo_corr_gender_age_gastric_LRTnofilterfoldchangeallsamples.pdf",
      width = 10,
      height = 8)
    ggplot(sibocorr, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    pdf("differentially_present_species by SIBO status_gastric samples.pdf",
      width = 8,
      height = 6)
    ggplot(data=sibocorr, aes(x = reorder(taxonomy, log2FoldChange), y=log2FoldChange)) +
     geom_bar(position="dodge",stat="identity", color="black") +
     coord_flip() +
     theme(axis.text.x = element_text(size=12))+
     theme(axis.text.y = element_text(size=12))+
...
#### make the Deseq analysis on gastric from Bangui samples and sibo on Species level gastric from Bangui samples, no filter for foldchange ####
    ...
    res = dffilteredgastricBangui %>%
      ...
      filter_prune( ) %>%
      deseq_fold_change( level = level
                             , factor_variables = c("sexe", "Run", "haz")
                             , continuous_variables = "age"
                             , variable_of_interest = variable_of_interest
                             , sig_level = 0.05)
    res$results_filtered
    ...
    corrsibogenderage_reduced_prevabdeseq <- sibocorr


    #' 2632

    pdf("differentially_present_taxa_Species_bysibo_corr_gender_age_gastric_LRTnofilterfoldchangeallsamplesBangui.pdf",
      width = 10,
      height = 8)
    ...
    pdf("differentially_present_species by SIBO status_gastric samplesBangui.pdf",
      width = 8,
      height = 6)
    ...
    pdf("differentially_present_taxa_Species_bysibo_corr_gender_age_haz_gastric_LRTnofilterfoldchangeallsamplesMada.pdf",
      width = 10,
      height = 8)
    ggplot(corrsibogenderage_reduced_prevabdeseq, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    pdf("differentially_present_species by SIBO status_gastric samplesTana.pdf",
      width = 8,
      height = 6)
    ...
    res = deseq_fold_change( dataset = dffilteredduodenal, level = level
                             , factor_variables = c("sexe", "Run" )
                             , continuous_variables = "age"
                             , variable_of_interest = variable_of_interest
                             , sig_level = 0.05)
    ...
    pdf("differentially_present_taxa_Genus_bycountryoforigin_corr_gender_age_duodenal_LRTallsamples.pdf",
      width = 10,
      height = 8)
    ggplot(payscorr, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    # make graph plot
    payscorr$taxonomy<- paste0(payscorr$Genus, "/", payscorr$Family)
    pdf("differentially_present_genera by country of origin_duodenal samples.pdf",
      width = 6,
      height = 1.3)

    differential_plot(payscorr, "Significantly different genera by country of origin in duodenal samples" )
    ...
    pdf("differentially_present_taxa_Species_bysibo_corr_gender_age_duodenal_LRTnofilterfoldchangeallsamples.pdf",
      width = 10,
      height = 8)
    ggplot(corrsibogenderage_reduced_prevabdeseq, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    corrsibogenderage_reduced_prevabdeseq$taxonomy<- paste0(corrsibogenderage_reduced_prevabdeseq$Species, "/", corrsibogenderage_reduced_prevabdeseq$Genus)
    pdf("differentially_present_species by SIBO status_duodenal samples.pdf",
      width = 8,
      height = 6)
    ...
    pdf("differentially_present_taxa_Genus_bysibo_corr_gender_age_duodenal_LRTnofilterfoldchangeallsamples.pdf",
      width = 10,
      height = 8)
    ...
    pdf("differentially_present_genera by SIBO status_duodenal samples.pdf",
      width = 8,
      height = 6)
    ...
    pdf("differentially_present_taxa_Species_bysibo_corr_gender_age_duodenal_LRTnofilterfoldchangeallsamplesBangui.pdf",
      width = 10,
      height = 8)
    ggplot(corrsibogenderage_reduced_prevabdeseq, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    corrsibogenderage_reduced_prevabdeseq$taxonomy<- paste0(corrsibogenderage_reduced_prevabdeseq$Species, "/", corrsibogenderage_reduced_prevabdeseq$Genus)
    pdf("differentially_present_species by SIBO status_duodenal samplesBangui.pdf",
      width = 8,
      height = 6)
   ...
   pdf("differentially_present_taxa_Genus_bysibo_corr_gender_age_duodenal_LRTnofilterfoldchangeallsamplesBangui.pdf",
      width = 10,
      height = 8)
    ggplot(corrsibogenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    corrsibogenderage_reduced_prevabdeseq$taxonomy<- paste0(corrsibogenderage_reduced_prevabdeseq$Genus, "/", corrsibogenderage_reduced_prevabdeseq$Family)
    pdf("differentially_present_genera by SIBO status_duodenal samplesBangui.pdf",
      width = 8,
      height = 6)
    ...
    pdf("differentially_present_taxa_Species_bysibo_corr_gender_age_duodenal_LRTnofilterfoldchangeallsamplesMada.pdf",
      width = 10,
      height = 8)
    ggplot(corrsibogenderage_reduced_prevabdeseq, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    corrsibogenderage_reduced_prevabdeseq$taxonomy<- paste0(corrsibogenderage_reduced_prevabdeseq$Species, "/", corrsibogenderage_reduced_prevabdeseq$Genus)
    pdf("differentially_present_species by SIBO status_duodenal samplesTana.pdf",
      width = 8,
      height = 6)
    ...
    pdf("differentially_present_taxa_Genus_bysibo_corr_gender_age_duodenal_LRTnofilterfoldchangeallsamplesMada.pdf",
      width = 10,
      height = 8)
    ggplot(corrsibogenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    corrsibogenderage_reduced_prevabdeseq$taxonomy<- paste0(corrsibogenderage_reduced_prevabdeseq$Genus, "/", corrsibogenderage_reduced_prevabdeseq$Family)
    pdf("differentially_present_genera by SIBO status_duodenal samplesTana.pdf",
      width = 8,
      height = 6)
  ...
  #### fecal samples ####
    ...
    pdf("differentially_present_taxa_Genus_bycountryoforigin_corr_gender_age_feces_LRTallsamples.pdf",
      width = 10,
      height = 8)
    ...
    pdf("differentially_present_genera by country of origin_fecal samples.pdf",
      width = 8,
      height =12
    )
    ...
    pdf("differentially_present_taxa_Genus_bystunted_stunted_vs_non.stunted_corr_gender_age_feces_LRTnofilterfoldchangeallsamples.pdf",
      width = 10,
      height = 8)
    ...
    pdf("differentially_present_genera by stunting status-merged dataset.pdf",
      width = 12,
      height = 8)
    ...
    pdf("differentially_present_taxa_Genus_bystunted_stunted_vs_non.stunted_corr_gender_age_feces_LRTnofilterfoldchangeallsamplesBangui.pdf",
      width = 10,
      height = 8)
    ggplot(corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    # make graph plot
    corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq$taxonomy<- paste0(corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq$Family, "/", corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq$Genus)
    pdf("differentially_present_genera by stunting status CAR.pdf",
      width = 12,
      height = 8)
    ggplot(data=corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq, aes(x = reorder(Genus, log2FoldChange), y=log2FoldChange)) +
     geom_bar(position="dodge",stat="identity", color="black") +
    ...
    pdf("differentially_present_taxa_Genus_bystunted_stunted_vs_non.stunted_corr_gender_age_feces_LRTnofilterfoldchangeallsamplesMada.pdf",
      width = 10,
      height = 8)
    ggplot(corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    # make graph plot
    corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq$taxonomy<- paste0(corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq$Family, "/", corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq$Genus)
    pdf("differentially_present_genera by stunting status Madagascar.pdf",
      width = 12,
      height = 8)
    ggplot(data=corrstunted_stunted_vs_non.stuntedgenderage_reduced_prevabdeseq, aes(x = reorder(Genus, log2FoldChange), y=log2FoldChange)) +
     geom_bar(position="dodge",stat="identity", color="black") +
    ...
    res = dffilteredfeces %>%
      ...
      #-- renaming elevated to 'too_high'
      change_factor_level( variable = variable_of_interest
                         , values = c("elevated")
                         , replacements = c("too_high") ) %>%
      filter_prune( ) %>%
      deseq_fold_change( level = level
                             , factor_variables = c("sexe", "Run", "stunted", "pays")
                             , continuous_variables = "age"
                             , variable_of_interest = variable_of_interest
                             , sig_level = 0.05)
    ...
    pdf("differentially_present_taxa_Genus_byalphaantitrypsinlevel_corr_gender_age_feces_LRTnofilterfoldchangeallsamples.pdf",
      width = 10,
      height = 8)
    ...
    pdf("differentially_present_genera by AAT status-merged dataset.pdf",
      width = 12,
      height = 8)
    ...
    res = dffilteredfecesBangui %>%
      ...
      filter_prune( ) %>%
      deseq_fold_change( level = level
                             , factor_variables = c("sexe", "Run", "stunted")
                             , continuous_variables = "age"
                             , variable_of_interest = variable_of_interest
                             , sig_level = 0.05)
    ...
    pdf("differentially_present_taxa_Genus_byalphaantitrypsinlevel_corr_gender_age_feces_LRTnofilterfoldchangeallsamplesBangui.pdf",
      width = 10,
      height = 8)
    ggplot(corralphaantitrypsinlevelgenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    # make graph plot
    corralphaantitrypsinlevelgenderage_reduced_prevabdeseq$taxonomy<- paste0(corralphaantitrypsinlevelgenderage_reduced_prevabdeseq$Family, "/", corralphaantitrypsinlevelgenderage_reduced_prevabdeseq$Genus)
    pdf("differentially_present_genera by AAT status-CAR.pdf",
      width = 15,
      height = 8)
    ggplot(data=corralphaantitrypsinlevelgenderage_reduced_prevabdeseq, aes(x = reorder(taxonomy, log2FoldChange), y=log2FoldChange)) +
     geom_bar(position="dodge",stat="identity", color="black") +
    ...
    pdf("differentially_present_taxa_Genus_byalphaantitrypsinlevel_corr_gender_age_feces_LRTnofilterfoldchangeallsamplesMada.pdf",
      width = 10,
      height = 8)
    ggplot(corralphaantitrypsinlevelgenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    # make graph plot
    corralphaantitrypsinlevelgenderage_reduced_prevabdeseq$taxonomy<- paste0(corralphaantitrypsinlevelgenderage_reduced_prevabdeseq$Family, "/", corralphaantitrypsinlevelgenderage_reduced_prevabdeseq$Genus)
    pdf("differentially_present_genera by AAT status-Mada.pdf",
      width = 12,
      height = 8)
    ggplot(data=corralphaantitrypsinlevelgenderage_reduced_prevabdeseq, aes(x = reorder(taxonomy, log2FoldChange), y=log2FoldChange)) +
     geom_bar(position="dodge",stat="identity", color="black") +
    ...
    pdf("differentially_present_taxa_Genus_bycalprotectinelevel_corr_gender_age_feces_LRTnofilterfoldchangeallsamples.pdf",
      width = 10,
      height = 8)
    ...
    pdf("differentially_present_genera by calpro status-merged dataset.pdf",
      width = 12,
      height = 8)
    ...
    pdf("differentially_present_taxa_Genus_bycalprotectinelevel_corr_gender_age_feces_LRTnofilterfoldchangeallsamplesBangui.pdf",
      width = 10,
      height = 8)
    ggplot(corrcalprotectinelevelgenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    # make graph plot
    corrcalprotectinelevelgenderage_reduced_prevabdeseq$taxonomy<- paste0(corrcalprotectinelevelgenderage_reduced_prevabdeseq$Family, "/", corrcalprotectinelevelgenderage_reduced_prevabdeseq$Genus)
    pdf("differentially_present_genera by calpro status-CAR.pdf",
      width = 12,
      height = 8)
    ggplot(data=corrcalprotectinelevelgenderage_reduced_prevabdeseq, aes(x = reorder(taxonomy, log2FoldChange), y=log2FoldChange)) +
     geom_bar(position="dodge",stat="identity", color="black") +
    ...
    pdf("differentially_present_taxa_Genus_bycalprotectinelevel_corr_gender_age_feces_LRTnofilterfoldchangeallsamplesMada.pdf",
      width = 10,
      height = 8)
    ggplot(corrcalprotectinelevelgenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
     scale_fill_manual(values = c("red", "blue", "green")) +
    ...
    # make graph plot
    corrcalprotectinelevelgenderage_reduced_prevabdeseq$taxonomy<- paste0(corrcalprotectinelevelgenderage_reduced_prevabdeseq$Family, "/", corrcalprotectinelevelgenderage_reduced_prevabdeseq$Genus)
    pdf("differentially_present_genera by calpro status-Mada.pdf",
      width = 12,
      height = 8)
    ggplot(data=corrcalprotectinelevelgenderage_reduced_prevabdeseq, aes(x = reorder(taxonomy, log2FoldChange), y=log2FoldChange)) +
     geom_bar(position="dodge",stat="identity", color="black") +
    ...
    corrsibogenderage_reduced_prevabdeseq <- payscorr
          ...
          pdf("differentially_present_taxa_Genus_bysibo_corr_gender_age_feces_LRTnofilterfoldchangeallsamples.pdf",
            width = 10,
            height = 8)
          ggplot(corrsibogenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
           scale_fill_manual(values = c("red", "blue", "green")) +
          ...
          pdf("differentially_present_genera in feces by sibo status-merged.pdf",
            width = 12,
            height = 8)
    ...
    corrsibogenderage_reduced_prevabdeseq <- payscorr
          ...
          pdf("differentially_present_taxa_Genus_bysibo_corr_gender_age_feces_LRTnofilterfoldchangeallsamplesBangui.pdf",
            width = 10,
            height = 8)
          ggplot(corrsibogenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
           scale_fill_manual(values = c("red", "blue", "green")) +
          ...
          # make graph plot
          corrsibogenderage_reduced_prevabdeseq$taxonomy<- paste0(corrsibogenderage_reduced_prevabdeseq$Family, "/", corrsibogenderage_reduced_prevabdeseq$Genus)
          pdf("differentially_present_genera in feces by sibo status-CAR.pdf",
            width = 12,
            height = 8)
          ggplot(data=corrsibogenderage_reduced_prevabdeseq, aes(x = reorder(taxonomy, log2FoldChange), y=log2FoldChange)) +
           geom_bar(position="dodge",stat="identity", color="black") +
    ...
    corrsibogenderage_reduced_prevabdeseq <- payscorr
          ...
          pdf("differentially_present_taxa_Genus_bysibo_corr_gender_age_feces_LRTnofilterfoldchangeallsamplesMada.pdf",
            width = 10,
            height = 8)
          ggplot(corrsibogenderage_reduced_prevabdeseq, aes(x=Genus, y=log2FoldChange, color=Class)) + geom_point(size=2) +
           scale_fill_manual(values = c("red", "blue", "green")) +
          ...
          pdf("differentially_present_genera in feces by sibo status-Mada.pdf",
            width = 12,
            height = 8)
          ggplot(data=corrsibogenderage_reduced_prevabdeseq, aes(x = reorder(taxonomy, log2FoldChange), y=log2FoldChange)) +
           geom_bar(position="dodge",stat="identity", color="black") +
...
#####' Analysis of cytokines in duodenal samples #####
    ### Make heatmaps of co-occuring cytokines in duodenum  ####
          ...
          pdf("heatmapcytokinesduodenum.pdf",
              width = 10,
              height = 10)
    ...
    ### Make heatmaps of co-occuring cytokines in duodenum CAR ####
          ...
          pdf("heatmapcytokinesduodenumCAR.pdf",
              width = 10,
              height = 10)
    ...
    ### Make heatmaps of co-occuring cytokines in duodenum Mada ####
          ...
          correlations = microbiome::associate(x, y, method = "spearman", mode = "matrix", p.adj.threshold = 0.05, n.signif = 1)
          correlation.table = microbiome::associate(x, y, method = "spearman", mode = "table", p.adj.threshold = 0.05, n.signif = 1)
          pdf("heatmapcytokinesduodenumMada.pdf",
              width = 10,
              height = 10)
          p = microbiome::heat(correlation.table, "X1", "X2", fill = "Correlation", star = "p.adj", p.adj.threshold = 0.05)
          print(p)
    ...
    ### Make heatmaps of co-occuring cytokines in duodenum by SIBO status ####
          ...
          correlations = microbiome::associate(x, y, method = "spearman", mode = "matrix", p.adj.threshold = 0.05, n.signif = 1)
          correlation.table = microbiome::associate(x, y, method = "spearman", mode = "table", p.adj.threshold = 0.05, n.signif = 1)
          pdf("heatmapcytokinesduodenumSIBO.pdf",
              width = 10,
              height = 10)
          p = microbiome::heat(correlation.table, "X1", "X2", fill = "Correlation", star = "p.adj", p.adj.threshold = 0.05)
          print(p)
          ...
          correlations = microbiome::associate(x, y, method = "spearman", mode = "matrix", p.adj.threshold = 0.05, n.signif = 1)
          correlation.table = microbiome::associate(x, y, method = "spearman", mode = "table", p.adj.threshold = 0.05, n.signif = 1)
          pdf("heatmapcytokinesduodenumnoSIBO.pdf",
              width = 10,
              height = 10)
          p = microbiome::heat(correlation.table, "X1", "X2", fill = "Correlation", star = "p.adj", p.adj.threshold = 0.05)
          print(p)
    ...
    ### Make heatmaps of co-occuring cytokines in duodenum with calprotectin and alphaantitrypsin in feces ####
          ...
          pdf("heatmapcytokinesduodenumcomparedtoAATCalproinfeces.pdf",
              width = 10,
              height = 10)
          p = microbiome::heat(correlation.table, "X1", "X2", fill = "Correlation", star = "p.adj", p.adj.threshold = 0.05)
          print(p)
    ...
    ### Make heatmaps of co-occuring cytokines in duodenum with calprotectin and alphaantitrypsin in feces RCA ####
          ...
          pdf("heatmapcytokinesduodenumcomparedtoAATCalproinfecesRCA.pdf",
              width = 10,
              height = 10)
          p = microbiome::heat(correlation.table, "X1", "X2", fill = "Correlation", star = "p.adj", p.adj.threshold = 0.05)
          print(p)
    ...
    ### Make heatmaps of co-occuring cytokines in duodenum with calprotectin and alphaantitrypsin in feces Mada ####
          ...
          correlations = microbiome::associate(x, y, method = "spearman", mode = "matrix", p.adj.threshold = 0.05, n.signif = 1)
          correlation.table = microbiome::associate(x, y, method = "spearman", mode = "table", p.adj.threshold = 0.05, n.signif = 1)
          pdf("heatmapcytokinesduodenumcomparedtoAATCalproinfecesMada.pdf",
              width = 10,
              height = 10)
          p = microbiome::heat(correlation.table, "X1", "X2", fill = "Correlation", star = "p.adj", p.adj.threshold = 0.05)
          print(p)
    ...
    ### Make heatmaps of co-occuring cytokines in duodenum with calprotectin and alphaantitrypsin in duodenum ####
          ...
          pdf("heatmapcytokinesduodenumcomparedtoAATCalproinduodenum.pdf",
              width = 10,
              height = 10)
          p = microbiome::heat(correlation.table, "X1", "X2", fill = "Correlation", star = "p.adj", p.adj.threshold = 0.05)
          print(p)
    ...
    ### Make heatmaps of co-occuring cytokines in duodenum with calprotectin and alphaantitrypsin in duodenum RCA ####
          ...
          pdf("heatmapcytokinesduodenumcomparedtoAATCalproinduodenumRCA.pdf",
              width = 10,
              height = 10)
          p = microbiome::heat(correlation.table, "X1", "X2", fill = "Correlation", star = "p.adj", p.adj.threshold = 0.05)
          print(p)
    ...
    ###' Make heatmaps of co-occuring cytokines in duodenum with calprotectin and alphaantitrypsin in duodenum Mada ####
          ...
          # Define data sets to cross-correlate with each other
          Dataduodenal_wilcox <- dffilteredduodenal_inflammation_Mada[, c(7:8, 10:39)] ## choose the columns you want to include
          dim(Dataduodenal_wilcox)
          Dataduodenal_wilcox2<-Dataduodenal_wilcox[complete.cases(Dataduodenal_wilcox)==TRUE, ]
          Dataduodenal_wilcox2[, 2]<-Dataduodenal_wilcox2[, 2]*1000  # as it cannot use values below 10
          Dataduodenal_wilcox2<-Dataduodenal_wilcox2[complete.cases(Dataduodenal_wilcox2)==TRUE, ]
          ...
          pdf("heatmapcytokinesduodenumcomparedtoAATCalproinduodenumMada.pdf",
              width = 10,
              height = 10)
          p = microbiome::heat(correlation.table, "X1", "X2", fill = "Correlation", star = "p.adj", p.adj.threshold = 0.05)
          print(p)
    ...
    ### PCA analysis on cytokines ####
          ...
          pdf("PlotindividusPcoACytokinesellipse.pdf",
              width = 10,
              height = 8)
          ...
          pdf("PlotindividusPcoACytokinespaysellipse.pdf",
              width = 10,
              height = 8)
          factoextra::fviz_pca_ind(res.pca,
    ...
    #### make a loop for logistic regressions ####
          ...
          logresults<- long %>%
            ...
            dplyr::do(tidy( stats::glm(as.factor(sibo) ~ value + batch + Country, .,  family = binomial))) %>%
    ...
    #### make a plot with the significant cytokines and their fold change ####
          ...
          #' 4217 make the graph
          pdf("differentially_present_cytokines_duodenum_sibo.noplate4.pdf",
              width = 4,
              height = 8)
    ...
    ##### rel. abundance of specific taxa, nutritional status, SIBO and inflammation markers on merged dataset ####
